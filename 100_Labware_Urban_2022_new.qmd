...
check2 <- check_lims_samples(df_samples_2022b)
check2$all
check2$biota
...
check1 <- get_nivabase_selection(
  "*", "LABWARE_IMPORT", "TEXT_ID", c("NR-2022-10791"), values_are_text = TRUE)   

check2 <- get_nivabase_selection(
  "*", "LABWARE_IMPORT", "REPORTED_NAME", c("Delta 15N"), values_are_text = TRUE, extra_sql = " and TEXT_ID like '%2022%'")   

table(check2$PROSJEKT)
table(check2$AQUAMONITOR_CODE)
table(is.na(check2$NUMERIC_ENTRY))  
...
check2 %>%
  count(substr(PROSJEKT, 1, 50), AQUAMONITOR_CODE, is.na(NUMERIC_ENTRY))

check2 %>% filter(!PROSJEKT %in% 'O 210200.ANAIN') %>% View("ch2")

```

#### Bile / galle samples: PFAS     

```{r}

# 2023
textid <- df_samples_all %>%
  filter(TISSUE == "BI-Galle" & year_from_textid == 2023) %>%
  pull(TEXT_ID)
check <- get_nivabase_selection(
  "*", "LABWARE_IMPORT", "TEXT_ID", textid, values_are_text = TRUE)   
nrow(check)

# 2021
textid <- df_samples_all %>%
  filter(TISSUE == "BI-Galle" & year_from_textid == 2021) %>%
  pull(TEXT_ID)
check <- get_nivabase_selection(
  "*", "LABWARE_IMPORT", "TEXT_ID", textid, values_are_text = TRUE)   
nrow(check)
...
table(check$AQUAMONITOR_CODE)  
table(check$REPORTED_NAME)  
table(check$ENTRY_QUALIFIER)  
