...
check1 <- dat_vm %>%
    xtabs(~Parameter_id + Medium_navn, .)
# check2 <- dat_vm %>%
#     xtabs(~Parameter_navn + Medium_navn, .)

head(check1) %>% kable(format = "simple")
...
* This was used before we did the particle 
...
- Some extra treatment needed water samples   
...
- taking into account water fraction + filtered fraction   
- Usig PARAM for parameter name (derived from NAME; particle fraction when NAME ends in '-P'  

```{r}

dat_nb_water %>%
  distinct(STATION_NAME, PARAM, Fraction, Year) %>%  
  xtabs(~addNA(STATION_NAME) + Year + Fraction, .)    

# List all parameters by year  
# xtabs(~PARAM + year(SAMPLE_DATE), dat_nb_water)
