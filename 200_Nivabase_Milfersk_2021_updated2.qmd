...
df_specimens <- get_specimens_from_stationdata(df_stations, years = current_year)
message("No. of specimens, biota: ", nrow(df_specimens))
dat_nb_biota <- get_biota_chemistry(current_year, df_specimens, df_stations, report_samples = TRUE)
...
dat_nb_water <- get_water_chemistry_from_stations(unique(df_samples_water$STATION_ID), current_year)
...
dat_nb_sediment <- get_sediment_chemistry_from_stations(df_stations$STATION_ID, years = current_year)
...
dat_nb_biota %>%
  count(STATION_ID, STATION_CODE, STATION_NAME, TISSUE_NAME)

```
### Check dates    
```{r}

df_date <- dat_nb_biota %>%
  count(STATION_ID, STATION_CODE, STATION_NAME, SAMPLE_DATE) %>%
  arrange(SAMPLE_DATE)

df_date

ggplot(dat_nb_biota, aes(SAMPLE_DATE)) +
  geom_histogram()
